            ...
            <?php foreach ($muebles as $m): ?>
                <div class="col-md-4">
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($m->getCategoria()->getNombre()) ?></h5>
                            <p class="card-text"><?= nl2br(htmlspecialchars(substr($m->getDescripcion(), 0, 100))) ?>...</p>
                            <p><strong>Dimensiones:</strong> <?= $m->getAncho() ?> x <?= $m->getAlto() ?> x
                                <?= $m->getLargo() ?> cm<br>
                                <strong>Peso:</strong> <?= $m->getPeso() ?? 'No especificado' ?> kg
                            </p>
                        </div>
                    </div>
                </div>
